    ...
    - To resume an existing response: GET /v1/responses/{response_id}?stream=true&starting_after=N.

    ## Stream options
    - stream_options.include_obfuscation (default true) adds an obfuscation field to delta events to normalize payload size.

    ## Event families (non-exhaustive)
    Lifecycle:
    - response.created, response.in_progress, response.completed, response.failed, response.incomplete, response.error
